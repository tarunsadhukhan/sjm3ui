...
import type { QrCv15aSetup, QrCv15aTableRow } from "./types";
...
export default function QrCv15aSqcPage() {
	...
	// ── Setup: machines + yarn items + saved tests for the date ────────────────
	...
	React.useEffect(() => {
		...
		const url = `${apiRoutesPortalMasters.QR_CV_15A_SETUP}?co_id=${coId}&entry_date=${entryDate}&branch_id=${branchId}`;
		void fetchWithCookie<{ data: QrCv15aSetup }>(url, "GET").then(({ data, error }) => {
			if (cancelled) return;
			if (error) {
				setSetupError(error);
				setSetup(null);
	...
	// ── History: paginated table of all saved tests (branch-scoped) ────────────
	const [histRows, setHistRows] = React.useState<QrCv15aTableRow[]>([]);
	...
	React.useEffect(() => {
		...
		void fetchWithCookie<{ data: QrCv15aTableRow[]; total: number }>(
			`${apiRoutesPortalMasters.QR_CV_15A_TABLE}?${params}`,
			"GET"
		).then(({ data, error }) => {
			if (cancelled) return;
			if (error || !data) {
	...
	const handleHistoryDelete = React.useCallback(
		async (id: number) => {
			...
			const { error } = await fetchWithCookie<{ data: { message: string } }>(
				`${apiRoutesPortalMasters.QR_CV_15A_DELETE}/${id}?co_id=${coId}`,
				"DELETE"
			);
			if (error) {
				setSnack(error);
	...
	const histColumns = React.useMemo<GridColDef<QrCv15aTableRow>[]>(
		() => [
			{
				field: "actions",
				headerName: "",
				...
				renderCell: (params) => (
					<Tooltip title="Delete test">
						<IconButton
							size="small"
							color="error"
							onClick={() => handleHistoryDelete(params.row.qr_cv_15a_id)}
			...
			},
			{
				...
				width: 140,
				valueGetter: (_value, row) => row.drawing_machine_name ?? "—",
			},
			{
				field: "machine_name",
	...
	return (
		<Box sx={{ p: { xs: 1.5, md: 3 } }}>
			...
			<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
				R-08-15A — Yarn QR% &amp; CV% special-purpose test (12 b/s readings per test).
			</Typography>
			...
			{branchId == null ? (
				<Alert severity="info">Select a branch to load QR-CV special test data.</Alert>
			) : (
				<Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
					...
					<Box>
						...
						<TextField
							size="small"
							label="Search"
							placeholder="Quality or machine"
							value={search}
							onChange={(e) => {
