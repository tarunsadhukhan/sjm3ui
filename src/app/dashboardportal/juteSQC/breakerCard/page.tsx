...
import type { BreakerCardSetup, ByDateResponse } from "./types";

const EMPTY_DAY: ByDateResponse = { rows: [], grand_averages: [] };
...
export default function BreakerCardSqcPage() {
	...
	// Setup (spells / breaker-card machines / batch plans)
	...
	React.useEffect(() => {
		...
		void fetchWithCookie<{ data: BreakerCardSetup }>(
			`${apiRoutesPortalMasters.BREAKER_CARD_SWT_SETUP}?co_id=${coId}&branch_id=${branchId}`,
			"GET"
		).then(({ data, error }) => {
			if (cancelled) return;
			if (error || !data) {
	...
	// Day view (rows + per-batch grand averages), refetched on save/delete via version bump.
	...
	React.useEffect(() => {
		...
		void fetchWithCookie<{ data: ByDateResponse }>(
			`${apiRoutesPortalMasters.BREAKER_CARD_SWT_BY_DATE}?co_id=${coId}&entry_date=${entryDate}&branch_id=${branchId}`,
			"GET"
		).then(({ data, error }) => {
			if (cancelled) return;
			setDayData(
	...
	return (
		<Box sx={{ p: { xs: 1.5, md: 3 } }}>
			...
			<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
				R-08-05/06/07 — per (machine, spell, batch) reading-set: 4 sliver-cut weights + 4 MR%,
				moisture-corrected server-side with per-batch grand averages.
			</Typography>
