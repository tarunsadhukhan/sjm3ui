...
import type { BreakerCardSetup } from "../types";
...
const SAMPLE_SIZE = 4;
// Breaker default STD MR% (batch carries no single std) — preview only, server recomputes.
const STD_MR_PCT = 16;
...
export default function EntryForm({ coId, branchId, entryDate, setup, onSaved }: Props) {
	...
	const formInvalid = !mcId || !spellId || !batchPlanId || preview == null;

	const handleSave = async () => {
		if (formInvalid) {
			setError(
				"Select machine, spell and batch, and enter all 4 weight + 4 MR% readings (weights > 0)."
			);
			return;
		}
		setSaving(true);
		...
		const body = {
			...
			// ponytail: one row per save (backend accepts many); card_side omitted — server defaults COARSE.
			rows: [
				{
					mc_id: Number(mcId),
		...
		const { error: err } = await fetchWithCookie<{ data: { count: number } }>(
			apiRoutesPortalMasters.BREAKER_CARD_SWT_CREATE,
			"POST",
			body
		);
		setSaving(false);
		...
		setSnack("Breaker card reading saved");
		// Keep machine/spell/batch so the inspector can punch many sets quickly.
	...
	return (
		<Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
			...
			{/* Live preview — corrected to STD MR% 16 (batch has no single std) */}
			<Box
				sx={{
					display: "grid",
					gap: 2,
