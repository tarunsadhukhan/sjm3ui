...
import type { PagedResponse, SliverWtRow } from "../types/spreaderSqcTypes";
...
/**
 * R-08-03 read side: today's samples (by-date, delete + detail), plus the
 * paginated all-dates history (search + detail via by_id + delete).
 */
export default function SliverWtGrid({ coId, rows, loading, refreshKey, onChanged }: Props) {
	const [snack, setSnack] = React.useState<string | null>(null);
	const [detail, setDetail] = React.useState<SliverWtRow | null>(null);

	// ── History table (server-paginated) ──
	const [histRows, setHistRows] = React.useState<SliverWtRow[]>([]);
	...
	React.useEffect(() => {
		...
		void fetchWithCookie<PagedResponse<SliverWtRow>>(
			`${apiRoutesPortalMasters.SPREADER_SLIVER_WT_TABLE}?${params.toString()}`,
			"GET"
		).then(({ data, error }) => {
			if (cancelled) return;
			if (!error && data) {
	...
	const handleDelete = React.useCallback(
		async (id: number) => {
			if (!confirm(`Delete sliver weight sample #${id}?`)) return;
			const { error } = await fetchWithCookie(
				`${apiRoutesPortalMasters.SPREADER_SLIVER_WT_DELETE}/${id}`,
				"DELETE"
			);
			if (error) {
				setSnack(error);
	...
	const handleViewById = React.useCallback(
		async (id: number) => {
			const { data, error } = await fetchWithCookie<{ data: SliverWtRow }>(
				`${apiRoutesPortalMasters.SPREADER_SLIVER_WT_BY_ID}?id=${id}&co_id=${coId}`,
				"GET"
			);
			if (error || !data?.data) {
	...
	const statCols = React.useMemo<GridColDef<SliverWtRow>[]>(
		() => [
			{
				field: "spell_code",
				headerName: "Spell",
			...
			{ field: "category", headerName: "Category", width: 120, valueGetter: (_v, row) => row.category ?? "—" },
			{
				field: "machine_name",
				headerName: "Machine",
				width: 160,
	...
	const dayCols = React.useMemo<GridColDef<SliverWtRow>[]>(
		() => [
			{
				field: "actions",
				headerName: "",
	...
	const histCols = React.useMemo<GridColDef<SliverWtRow>[]>(
		() => [
			{
				field: "actions",
				headerName: "",
				...
				renderCell: (params) => (
					<Box>
						<Tooltip title="View readings">
							<IconButton size="small" onClick={() => handleViewById(params.row.spreader_sliver_wt_id)}>
								<ViewIcon size={16} />
							</IconButton>
						</Tooltip>
						<Tooltip title="Delete sample">
	...
	const readings = detail?.observed_weights ?? [];
		...
		<Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
			<Box>
				...
				<Box sx={{ width: "100%", overflowX: "auto" }}>
					<DataGrid
						autoHeight
						rows={rows}
						getRowId={(row) => row.spreader_sliver_wt_id}
						columns={dayCols}
						loading={loading}
			...
			<Box>
				<Box sx={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap", mb: 1 }}>
					...
					<TextField
						size="small"
						placeholder="Search category / quality / machine"
						value={searchInput}
			...
			<Dialog open={!!detail} onClose={() => setDetail(null)} maxWidth="sm" fullWidth>
				<DialogTitle>
					Sliver Weight Sample #{detail?.spreader_sliver_wt_id}
					...
					{detail ? (
						<Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
							<Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
								...
								<Chip size="small" label={`Category: ${detail.category ?? "—"}`} />
								...
								<Chip
									size="small"
									label={`Sample: ${fmt(detail.sample_length_yds, 1)} yds · ${detail.weight_basis ?? "—"}`}
								/>
							</Box>
							...
							<TableContainer component={Paper} variant="outlined">
								<Table size="small">
									<TableHead>
										<TableRow>
											<TableCell>#</TableCell>
											<TableCell align="right">Sliver Wt</TableCell>
							...
							<Paper variant="outlined" sx={{ p: 2 }}>
								...
								<Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
									<Chip size="small" label={`Avg Obs: ${fmt(detail.calc_avg_obs)}`} />
									<Chip size="small" label={`Avg MR%: ${fmt(detail.calc_avg_mr)}`} />
									<Chip size="small" label={`Avg Corr: ${fmt(detail.calc_avg_corr)}`} />
									<Chip size="small" label={`StDev: ${fmt(detail.calc_stdev, 3)}`} />
