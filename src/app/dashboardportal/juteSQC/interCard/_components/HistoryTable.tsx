...
import { fmt, fmtCv, type CardSliverRow, type CardSliverTableRow } from "../types";
...
export default function HistoryTable({ coId, branchId, version, onChanged }: Props) {
	const [rows, setRows] = React.useState<CardSliverTableRow[]>([]);
	...
	// View dialog
	const [viewRow, setViewRow] = React.useState<CardSliverRow | null>(null);
	...
	React.useEffect(() => {
		...
		void fetchWithCookie<{ data: CardSliverTableRow[]; total: number }>(
			`${apiRoutesPortalMasters.CARD_SLIVER_WT_TABLE}?${params}`,
			"GET"
		).then(({ data, error }) => {
			if (cancelled) return;
			if (error || !data) {
	...
	const handleView = React.useCallback(
		async (id: number) => {
			setViewOpen(true);
			setViewLoading(true);
			const { data, error } = await fetchWithCookie<{ data: CardSliverRow }>(
				`${apiRoutesPortalMasters.CARD_SLIVER_WT_BY_ID}?id=${id}&co_id=${coId}`,
				"GET"
			);
	...
	const columns = React.useMemo<GridColDef<CardSliverTableRow>[]>(
		() => [
			{
				field: "actions",
				headerName: "",
				...
				renderCell: (params) => (
					<>
						<Tooltip title="View reading">
							<IconButton
								size="small"
								onClick={() => handleView(params.row.card_sliver_wt_id)}
							>
								<ViewIcon size={16} />
							</IconButton>
						</Tooltip>
