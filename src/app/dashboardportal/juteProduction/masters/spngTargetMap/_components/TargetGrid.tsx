...
export default function TargetGrid({
	...
	// Reseed edits whenever a fresh grid loads (date / type / role change).
	React.useEffect(() => {
		setEdits(baseline);
	}, [baseline]);
	...
	if (grid.rows.length === 0) {
		return <Alert severity="info">No machines / yarns found for this company.</Alert>;
	}

	if (grid.params.length === 0) {
		return <Alert severity="info">No parameters apply to this Type / Role combination.</Alert>;
	}
				...
				<Table size="small" sx={{ minWidth: 600 }}>
					<TableHead>
						<TableRow>
							...
							<TableCell>Machine / Yarn</TableCell>
							{grid.params.map((p) => (
