    ...
    const fetchCompanyData = async () => {
      ...
      try {
        ...
        if (result.status === 401 || result.status === 403) {
          // Auto-redirect to login on a backend auth error is DISABLED.
          // Previously this cleared the cache and did `window.location.href = "/"`,
          // which caused the page to refresh/bounce to login whenever the backend
          // rejected the session. We now just stop loading and leave any cached
          // sidebar data in place — no refresh, no redirect.
          console.warn("Portal menu request returned", result.status, "— not redirecting (auto-logout disabled)");
          return;
        }
